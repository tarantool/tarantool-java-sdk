---
title: TarantoolDB. Примеры
---

## Пример 1. Запуск кластера TDB без определения конфигурации

В данном примере рассматривается случай запуска кластера `TDB 2.x` со стандартным набором настроек,
с определением кол-ва маршрутизаторов, шардов и реплик в них. Стандартная конфигурация выглядит
следующим образом (генерируется автоматически классов `TDB2Cluster`):
...
В данном шаге демонстрируется простейший запуск кластера TDB 2.x с указанием кол-ва различных 
компонентов кластера (маршрутизаторы, шарды, реплики) без передачи пользовательских настроек и
миграций.
...
В результате работы вы должны получить следующие логи:
...
В данном шаге демонстрируется способ применения миграций в тестовом кластере. Топология кластера
TDB 2.x аналогична предыдущему примеру.

Сначала определим миграции. Для этого создадим директорию `migrations/scenario` (в произвольном
месте). В директории `migrations/scenario` определим миграцию `001_kv.lua`:
...
Запустим кластер с передачей миграций и удостоверимся, что миграции применились:
...
## Пример 2. Запуск кластера с пользовательскими настройками

Для определения настроек кластера можно воспользоваться классом `Tarantool3Configuration`.

Допустим, что вам нужно запустить кластер с 2 маршрутизаторами (router), 2 репликасетами хранилищ 
(storage), в которых будет две реплики (master и slave):
...
Графическое представление топологии:
...
Определим конфигурацию выше через `Tarantool3Configuration`:
...
## Пример 3. Запуск кластера с пользовательскими настройками

Предположим, что у вас уже есть файл с конфигурацией:
...
Для того чтобы использовать его, воспользуемся `YamlMapper`:
...
???+ warning "Важно"
    ...
    * Для работы `testcontainers` конфигурация должна содержать пользователя `test-super` с паролем
      `secret` и ролью `super`
    * `iproto.listen.uri` должен иметь `hostname` идентичный имени экземпляра
      `instances.<instance_name>`
    * Логин и пароль от web UI `TCM`:
        * Логин: `admin`
        * Пароль: `secret`
