...
public class TarantoolJacksonMappingWithTargetClass extends TarantoolJacksonMappingWithoutTargetType {
  ...
  public static <T> CompletableFuture<TarantoolResponse<List<T>>> convertFutureResult(
    ...
    return future.thenApply(
        response -> readResponse(response, entity)
    );
  }

  public static <T> TarantoolResponse<List<T>> readResponse(IProtoResponse response, Class<T> entity) {
  ...
  public static <T> TarantoolResponse<List<Tuple<T>>> readSpaceData(IProtoResponse response, Class<T> entity) {
    return new TarantoolResponse<>(
        readData(
            response,
            wrapIntoList(
                wrapIntoTuple(entity)
            )
        ),
        getFormats(response)
    );
  ...
  public static <T> CompletableFuture<Tuple<T>> convertCrudSingleResultFuture(
    ...
    return future
        .thenApply(
            response -> readCrudSingleResultData(response, entity)
        )
        .thenApply(
            response -> getTupleWithInjectedFormat(response)
        );
  ...
  public static <T> SelectResponse<List<Tuple<T>>> readSelectResult(IProtoResponse response, Class<T> entity) {
  ...
  private static <T> SelectResponse<List<Tuple<T>>> injectFormatIntoTuples(SelectResponse<List<Tuple<T>>> resp) {
    Map<Integer, List<Field>> formats = resp.getFormats();
    if (!formats.isEmpty()) {
  ...
  public static <T> TarantoolResponse<List<Tuple<T>>> readCrudSelectResult(IProtoResponse response, Class<T> entity) {
    return new TarantoolResponse<>(
        getRows(
            readData(
                response,
  ...
  public static <T> CompletableFuture<CrudBatchResponse<List<Tuple<T>>>> convertCrudBatchResultFuture(
      CompletableFuture<IProtoResponse> future,
      Class<T> entity) {
    return future
  ...
  public static <T> TarantoolResponse<CrudBatchResponse<List<Tuple<T>>>> readCrudBatchResult(IProtoResponse response,
      Class<T> entity) {
    return new TarantoolResponse<>(
        readData(
            response,
  ...
  private static <T> CrudBatchResponse<List<Tuple<T>>> getBatchTuplesWithInjectedFormat(TarantoolResponse<CrudBatchResponse<List<Tuple<T>>>> resp) {
    Map<Integer, List<Field>> formats = resp.getFormats();
  ...
  private static <T> List<Tuple<T>> getTuplesWithInjectedFormat(TarantoolResponse<List<Tuple<T>>> resp) {
    Map<Integer, List<Field>> formats = resp.getFormats();
