...
/**
 * Class that helps get parameter from {@link Tarantool3Configuration}. Use extractors with
 * different scopes to get parameter from that scopes:
 *
 * <ol>
 *   <li>Use {@link #globalExtractor} to get parameter from global scope
 *   <li>Use {@link #groupExtractor} to get parameter from groups scope
 *   <li>Use {@link #replicasetExtractor} to get parameter from replicaset scope
 *   <li>Use {@link #instanceExtractor} to get parameter from instance scope
 * </ol>
 *
...
public class ParameterExtractor<R> {
  ...
  /**
   * Extractor to get parameters from global scope of configuration. <b>Use to extract parameters
   * from global scope only</b>. If parameter is not present in scope return {@link
   * Optional#empty()} instance.
   */
  private final Function<Tarantool3Configuration, Optional<R>> globalExtractor;

  /**
   * Extractor to get parameters from groups scope of configuration. <b>Use to extract parameters
   * from groups scope only</b>. If parameter is not present in scope return {@link
   * Optional#empty()} instance.
   */
  private final Function<GroupsProperty, Optional<R>> groupExtractor;

  /**
   * Extractor to get parameters from replicaset scope of configuration. <b>Use to extract
   * parameters from replicaset scope only</b>. If parameter is not present in scope return {@link
   * Optional#empty()} instance.
   */
  private final Function<ReplicasetsProperty, Optional<R>> replicasetExtractor;

  /**
   * Extractor to get parameters from instance scope of configuration. <b>Use to extract parameters
   * from instance scope only</b>. If parameter is not present in scope return {@link
   * Optional#empty()} instance.
   */
  private final Function<InstancesProperty, Optional<R>> instanceExtractor;

  /**
   * {@link ConfigCombiner} function to combine result returned from {@link #globalExtractor},
   * {@link #groupExtractor}, {@link #replicasetExtractor}, {@link #instanceExtractor}. Can use to
   * set priority for returning parameters.
   */
  private final ConfigCombiner<Optional<R>, Optional<R>, Optional<R>, Optional<R>, Optional<R>>
      combineFunction;
  ...
  public ParameterExtractor(
      ...
      ConfigCombiner<Optional<R>, Optional<R>, Optional<R>, Optional<R>, Optional<R>>
          combineFunction) {
  ...
  /**
   * Returns map with keys are Tarantool 3 instance names, values are extracted parameter. If
   * resulting parameter is not present or null, this parameter won't put into result map. Extracts
   * parameter for all instance in configuration.
   */
  ...
  /**
   * Same as {@link #getParameter(Tarantool3Configuration)}. Returns parameter for only passed
   * instance name.
   */
  public Optional<R> getParameter(String instanceName, Tarantool3Configuration config) {
    ...
    final Collection<GroupsProperty> groups =
        config
            .getGroups()
    ...
    for (GroupsProperty group : groups) {
      ...
      for (ReplicasetsProperty replicaset : replicasets) {
        ...
        final Map<String, InstancesProperty> additionalProperties =
            replicaset
                .getInstances()
                .orElseThrow(IllegalArgumentException::new)
                .getAdditionalProperties();
