...
/**
 * A waiting strategy that checks whether {@code Tarantool} is ready for interaction. Strategy is using {@code tt} to
 * execute {@code box.info.status} command. The passed {@code user} with {@code password} must have permission to
 * execute {@code box.info.status} command.
 */
public class Tarantool3WaitStrategy extends ShellStrategy {

  private int port = 3301;

  private static final String FORMAT = "if echo \"box.info.status\" | tt connect %s:%s@%s:%s -x lua | grep -q "
