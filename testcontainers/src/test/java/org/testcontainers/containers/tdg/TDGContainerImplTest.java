...
class TDGContainerImplTest {
  ...
  @Test
  @DisplayName("Проверьте, что переменные окружения передаются в контейнер между созданием экземпляра и до его запуска")
  void testToSetEnvs() throws IOException, InterruptedException {
    ...
    try (TDGContainer<TDGContainerImpl> c = TDGContainerImpl.builder(TDG_IMAGE).build()) {
      ...
      env.entrySet().stream().map(e -> e.getKey() + "=" + e.getValue()).forEach(s -> {
  ...
  void testPortsAreOpen(String advertiseUri) throws IOException {
    try (TDGContainer<TDGContainerImpl> c = TDGContainerImpl.builder(TDG_IMAGE)
        .withAdvertiseUri(advertiseUri).build()) {
      c.start();
