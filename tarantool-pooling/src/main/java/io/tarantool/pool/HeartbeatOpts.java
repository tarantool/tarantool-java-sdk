...
/**
 * This class represents a set of options used in heartbeats.
 *
 * <p>Heartbeat is a task executing regular pings to Tarantool node and analyzing results if pings
 * are successful or not. When a certain number of failed pings is reached, the heartbeat puts a
 * controlled connection into <b>INVALIDATED</b> state. Invalidated connection is not returned to
 * outer clients but heartbeat continues to ping. For example, isn't useful for following cases:
 *
 * <ul>
 *   <li>In case of tarantool client overload. When too many requests are run and buffers overflow
 *       on client or Tarantool sides it can help to unload connection - after some successful pings
 *       connection will be returned back to <b>ACTIVATED</b> state.
 *   <li>In cases of network or Tarantool outages. For example when node becomes stuck due to
 *       problem with TX thread (infinite loops, etc) heartbeat will receive failures for each ping
 *       to invalidated connections and after several attempts it will move connection to
 *       <b>KILLED</b> state and reconnect task will be run.
 * </ul>
 *
 * <p>Heartbeat doesn't react to any ping failure immediately, because it can lead to problems with
 * balancing and unstable work. Instead of it heartbeat uses sliding window to make reactions
 * smoother. Heartbeat analyzes last N pings and makes decision.
 *
 * <p>For example, {@link #windowSize} is 4, {@link #invalidationThreshold} is 2 and {@link
 * #deathThreshold} is 4. It means, that heartbeat after starting the heartbeat will wait for 4
 * pings and then make the first decision. If a count of failed pings is at least 2 then heartbeat
 * will invalidate this connection. Each failed ping in this state will be counted for further
 * comparison with death threshold. If a count of failed pings within the window becomes lower and
 * death threshold is not reached, connection is moved back to activated state and dead pings
 * counter resets. If the death threshold is reached, the connection will be closed and reconnect
 * task will be run.
 *
...
public final class HeartbeatOpts {
   ...
   *
   * <p><i><b>Default</b></i>: {@code 2}.
   */
  private int invalidationThreshold;

  /**
   * Total count of pings which should be done.
   *
   * <p>When heartbeat starts it waits for accumulating window and then this windows slides for each
   * subsequent ping request. For example, size of window is 4 and it means that after start
   * heartbeat should execute 4 ping requests and after some decision depending on results of ping
   * will be made.
   *
   * <p><i><b>Default</b></i>: {@code 4}.
   */
  private int windowSize;

  /**
   * Count of failures to move connection to KILLED state from INVALIDATED.
   *
   * <p>After invalidation heartbeat will continue to ping this connection and results of pings will
   * be analyzed. If this amount of failed pings is reached then connection will be killed and
   * reopened.
   *
   * <p><i><b>Default</b></i>: {@code 4}.
   */
   ...
   *
   * <p>If tarantool raise exception or timeout happened then we accept it as failed ping. Also we
   * check response. If the response is not empty and doesn't consist true message inside we mark it
   * as failed ping. For example you can use such function:
   *
   * <blockquote>
   *
   * <pre>{@code
   ...
   *
   * <p><i><b>Default</b></i>: IPROTO_PING with iproto options as request timeout.
   */
  private BiFunction<IProtoClient, IProtoRequestOpts, CompletableFuture<IProtoResponse>>
      pingFunction;

  /**
   * Static method for getting default heartbeat options.
   *
  ...
  /**
   * Predefined function that could be helpful to determine whether crud role is up or not. For
   * example you can use such function:
   *
   * <blockquote>
   *
   * <pre>{@code
  ...
  public HeartbeatOpts withCrudHealthCheck() {
    return withPingFunction(
        (client, heartbeatPingOpts) ->
            client.eval(
                "return rawget(_G, 'crud') ~= nil", ValueFactory.emptyArray(), heartbeatPingOpts));
  }
   ...
   */
  public BiFunction<IProtoClient, IProtoRequestOpts, CompletableFuture<IProtoResponse>>
      getPingFunction() {
    return pingFunction;
  }
}
